...
async function json(name){const packed=name==='reviewed.json';const response=await fetch('./'+name+(packed?'.gz':''));if(!response.ok)throw new Error('Dataset request failed');if(!packed)return response.json();const bytes=await response.arrayBuffer();const digest=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',bytes)),b=>b.toString(16).padStart(2,'0')).join('');if(digest!=='a15e433fb94a5ecea29fc8d33a78d91cdcd8256834264cb7e005891ce4383223')throw new Error('Dataset integrity mismatch');const stream=new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'));return new Response(stream).json()}
